...
import { Pencil, Plus, Eye, X, Send, PackageCheck, Wallet, Trash2, Loader2 } from "lucide-react";
...
import type { Purchase, PurchaseLine, PurchaseType, Product, Supplier, SettingsData, PaymentMethod } from "@/lib/types";
...
export function PurchasesClient({
  purchases, totalCount, suppliers, products, methods, settings, permissions,
}: {
  purchases: Purchase[];
  totalCount: number;
  suppliers: Supplier[];
...
function PurchaseRowActions({
  row, permissions, products, onView, onEdit, onPay, onReceive,
}: {
  row: Purchase; permissions: PermissionMatrix; products: Product[];
  onView: () => void; onEdit: () => void; onPay: () => void; onReceive: () => void;
}) {
...
function ReceiveDialog({
  ...
  return (
    <Dialog open onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>Receive PO {purchase.po_no}</DialogTitle>
          <DialogDescription>
            {anySerialTracked
              ? "Enter the serial number (and optional barcode) for each unit being received."
